...
- Все счетчики и диапазоны, кроме `sections[].count`, пересчитываются с учетом переданного `filters`.
- `sections[].count` всегда показывает количество активных товаров раздела и всех его дочерних разделов во всём каталоге; выбранные фильтры на него не влияют.
...
Для `choice` значение — объект варианта характеристики или массив таких объектов, если у характеристики включено несколько значений.

Поля варианта характеристики:
...
| `h1` | SEO H1 |
| `do_not_use_for_direct_url` | Не использовать для прямой ссылки |
...
| `logo` | Логотип |
| `priority` | Приоритет |
| `count` | Количество всех товаров производителя |
